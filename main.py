...
def is_valid_file(file):
    try:
        ...
        if ext == ".pdf":
            ...
            try:
                # Save to temp file and read paragraph count
...
def convert_to_pdf(uploaded_file_obj, ext, save_path):
    ...
    # Step 1: Save original file with its original extension
    original_ext_path = save_path  # should already have proper extension
    with open(original_ext_path, "wb") as f:
        f.write(uploaded_file_obj.read())

    # Step 2: Prepare PDF output path
    pdf_file_path = os.path.splitext(original_ext_path)[0] + ".pdf"
    
    # Step 3: Remove existing PDF if exists
    if os.path.exists(pdf_file_path):
        os.remove(pdf_file_path)

    try:
        if ext.lower() in [".jpg", ".jpeg", ".png"]:
            image = Image.open(original_ext_path).convert("RGB")
        ...
        with st.spinner("Preparing to process..."):
            ...
            try:
                ...
                with st.status("Extracting text from documents...", expanded=True) as status:
                    ...
                    txt_save_path = os.path.join(UPLOAD_FOLDER, f"extracted_text_{str(uuid.uuid4())[:8]}.txt")
                    with open(txt_save_path, "w", encoding="utf-8") as f:
                        f.write(combined_extracted_text)
                    st.success(f"Extracted text saved at `{txt_save_path}`")
